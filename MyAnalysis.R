#读取数据
algae <- read.table('D:\\R\\Analysis.txt',
header=F,
dec='.',
...
write.table(omiteddata,'D:\\R\\OmitedData.txt',col.names = F,row.names = F, quote = F)

#使用高频数据替换
library(DMwR)
preprocess2 = algae[-manyNAs(algae),]
preprocess2 = centralImputation(preprocess2)
write.table(preprocess2,'D:\\R\\CentralImputationData.txt',
...
write.table(preprocess3,'D:\\R\\linearDefaultData.txt',
col.names = F,row.names = F, quote = F)

#通过案例的相关性填补缺失值
preprocess4 = knnImputation(algae,k=10)
write.table(preprocess4,'D:\\R\\knnImputationData.txt',
col.names = F,row.names = F, quote = F)